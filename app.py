"""
Formula 1 · Performance & Economics
-----------------------------------
Streamlit dashboard over five linked files:
  race_results.csv        337 races, 2010-2026
  circuit_economics.csv   277 circuit-seasons (hosting fees, attendance)
  constructor_finances.csv 164 team-seasons (budgets, revenue, cost cap)
  driver_standings.csv    107 top-6 championship rows
  f1_business.csv          17 seasons of the sport's own P&L
...
if missing:
    st.markdown("<div class='masthead'><div class='eyebrow'><span>Setup</span></div>"
                "<h1>Missing <em>data files</em></h1></div>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with c1:
        ...
        fig.update_layout(yaxis_tickformat=".0%" if view == "Share of revenue" else "$,.0f",
                          xaxis_title="", yaxis_title="", legend=dict(orientation="h", y=1.14))
    ...
    with c2:
        ...
        fig.update_layout(title="Revenue earned per race", yaxis_title="$M per race", xaxis_title="")
    ...
    with c3:
        ...
        fig.update_layout(title="Streaming audience vs race audience",
                          ...
                          legend=dict(orientation="h", y=1.16), xaxis_title="")
...
with tabs[1]:
    ...
    with c1:
        ...
        fig.update_layout(xaxis_title="Operating budget ($M)", yaxis_title="Total revenue ($M)")
    ...
    with c2:
        ...
        fig.add_hline(y=0, line_color=MUTED, line_width=1)
        fig.update_layout(yaxis_title="Revenue − budget ($M)", xaxis_title="",
    ...
    with c3:
        ...
        fig.update_layout(xaxis_title="", yaxis_title="Operating budget ($M)",
    ...
    with c4:
        ...
        fig.update_layout(title="Grid inequality", yaxis_title="$M",
                          yaxis2=dict(title="×", overlaying="y", side="right", showgrid=False),
                          xaxis_title="", legend=dict(orientation="h", y=1.16))
    ...
    with c5:
        ...
        fig.update_layout(coloraxis_colorbar=dict(title="Margin", tickformat=".0%"))
    ...
    with c6:
        ...
        fig.update_layout(barmode="stack", xaxis_title="$M", yaxis_title="",
...
with tabs[2]:
    ...
    with c1:
        ...
        fig.update_layout(xaxis_title="Hosting fee ($M est.)", yaxis_title="Weekend attendance (k)")
    ...
    with c2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=cmp_["Era"], y=cmp_["Fee"], name="Avg fee ($M)", marker_color=AMBER,
                             text=cmp_["Fee"].round(1), textposition="outside"))
        fig.add_trace(go.Bar(x=cmp_["Era"], y=cmp_["Attendance"] / 10, name="Avg attendance (k ÷10)",
                             marker_color=TYRE_RED, text=(cmp_["Attendance"]).round(0), textposition="outside"))
        fig.update_layout(title="New venues vs established ones", barmode="group",
                          yaxis_title="", legend=dict(orientation="h", y=1.14))
    ...
    with c3:
        ...
        fig.update_layout(yaxis={"categoryorder": "total ascending"}, yaxis_title="",
                          xaxis_title="Hosting fee per attendee ($)", coloraxis_showscale=False)
...
with tabs[3]:
    ...
    with c1:
        ...
        fig.update_layout(xaxis_title="", yaxis_title="Wins", legend=dict(orientation="h", y=1.14))
...
with tabs[4]:
    ...
    with c2:
        ...
        fig.update_layout(coloraxis_showscale=False, xaxis_title="Budget rank in that season (1 = highest)",
                          yaxis_title="Titles won")
    ...
    with c3:
        ...
        fig.update_layout(title=f"Indexed to {base_year} = 100", xaxis_title="",
                          yaxis_title="Index", legend=dict(orientation="h", y=1.16))
...
with tabs[5]:
    ...
    with c2:
        ...
        fig.update_layout(title="Reported revenue vs its own components",
                          xaxis_title="", yaxis_title="$M", legend=dict(orientation="h", y=1.14))
